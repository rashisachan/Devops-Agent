...
# Free, fast LLM via Groq
llm = ChatGroq(
    model="llama3-70b-8192",   # free model
    api_key=os.getenv("GROQ_API_KEY"),
    temperature=0.1             # low = more deterministic for code
)

# ─── Node 1: Read the issue ───────────────────────────────────────
...
# ─── Node 2: Clone repo & find files ─────────────────────────────
def find_files_node(state: dict) -> dict:
    trace = state["trace"]
    trace.add("Clone Repo", "Cloning repository locally...")
    
    repo = clone_repo("./cloned_repo")
    state["repo_path"] = "./cloned_repo"
    
    candidates = find_relevant_files(
        state["repo_path"], state["issue_summary"]
    )
    state["candidate_files"] = candidates
    trace.add("Find Files", f"Found {len(candidates)} candidate files", 
              str(candidates))
    return state

# ─── Node 3: LLM picks the buggy file ───────────────────────────
def identify_bug_node(state: dict) -> dict:
    ...
    # Ask LLM which file has the bug
    files_text = "\n\n".join(
        [f"### {fp}\n```python\n{content}\n```" 
...
{files_text}""")
    ...
    trace.add("Identify Bug", 
              f"LLM identified bug in: {state['buggy_file']}",
...
# ─── Node 5: Run the fixed code ──────────────────────────────────
def run_code_node(state: dict) -> dict:
    trace = state["trace"]
    
    # Write the fix temporarily
    ...
    # If it failed, restore original for next iteration
...
def create_pr_node(state: dict) -> dict:
    ...
    commit_and_push(
        state["repo_path"], branch, 
        state["buggy_file"], state["fixed_code"]
    )
    ...
    state["pr_url"] = pr_url
    trace.add("PR Created", f"Pull Request opened!", pr_url)
    return state